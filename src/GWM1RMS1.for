...
C-----------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ONE,ZERO,BIGINF,GWMOUT
      INTEGER(I4B),INTENT(IN)::M,NV,NDV,LPITMAX
...
C-----PERFORM PHASE 1 - SOLVE FROM AN ALL ARTIFICIAL START
	CALL FNDFES(AMAT,COST,BNDS,RHS,M,NV,NDV,LPITMAX,IFLG)
      IF(IFLG.EQ.0)THEN
C-------SOLUTION FOUND TO PHASE 1.
C
C-------PERFORM PHASE 2 - SOLVE FROM THE CURRENT FEASIBLE POINT
        CALL SLVLP (RHS,BNDS,M,NV,LPITMAX,IFLG)
C
        IF(IFLG.EQ.0)THEN
...
C-----------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ZERO,SMALLEPS
...
C---------COMPARE WITH CURRENT SMALLEST NON-BASIC CHANGE
  ...
  200   ENDDO
      ENDIF
C
C-----TESTS FOR UNBOUNDED PROBLEM
      IF(ILEAVE.LT.0 .AND. BNDS(IRTIC(ABS(ILEAVE))).EQ.BIGINF)THEN
...
C-----------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ZERO
      USE GWM1DCV1, ONLY : NBVAR
      USE GWM1RMS1, ONLY : CSTROR,NONLIN,RANGEFLG,LASTLP
      INTEGER(I4B),INTENT(IN)::M,NV
...
C-----------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ONE,ZERO,SMALLEPS,BIGINF 
      USE GWM1RMS1, ONLY : CSTREL,CSTRLL,CSTREU,CSTRLU,CSTRLB,CSTRUB,
     &                     CSTROR
      INTEGER(I4B),INTENT(IN)::M,NV
...
C-----------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ONE,ZERO,SMALLEPS,BIGINF
      USE GWM1RMS1, ONLY : CSTROR,RHSREL,RHSRLL,RHSREU,RHSRLU,RHSRLB,
     1                     RHSRUB,RHSROR   
      INTEGER(I4B),INTENT(IN)::M,NV
...
C---------PERFORM RATIO TEST ON ALL NONBASICS
 ...
 440        ENDDO
C
C-------CHECK IF NONBASIC HITS ITS OTHER BOUND FIRST
C        SET IENT = 0 TO IDENTIFY NONBASIC HITTING ITS OTHER BOUND
c           CALL MINXI(CKU, BNDS(J), IENT, 0)
...
C---------- BASIC LEAVES AT LOWER BOUND
              ...
              ENDIF
C
C-----------TEST IF BASIC LEAVES AT UPPER BOUND
            ELSEIF(UPRBND) THEN
...
C
C-----------------------------------------------------------------------
      USE GWM1DCV1, ONLY : NFVAR,NBVAR,FVON,BVLIST,BVNLIST
      USE GWM1BAS1, ONLY : GWMOUT,BIGINF,ZERO
      USE GWM1RMS1, ONLY : BBITMAX,LPITMAX,BBITPRT,LASTLP
...
C-------THE INCUMBENT IS FEASIBLE AND BEST; RERUN OPTIMAL SOLUTION
          WRITE(GWMOUT,*,ERR=990)
          WRITE(GWMOUT,*,ERR=990)J,' RELAXED PROBLEMS WERE SOLVED.'
          J = INC
C
...
C-----------------------------------------------------------------------
      USE GWM1DCV1, ONLY : NFVAR,NBVAR,BVLIST,BVNLIST
      USE GWM1BAS1, ONLY : ZERO,BIGINF,GWMOUT
      USE GWM1RMS1, ONLY : BBITMAX,BBITPRT
...
C         PREVIOUS UNFATHOMED SUBPROBLEMS
          DO 200 I=1,J-1
            IF(IRLX(I).EQ.1 .AND. OBJS(I).GE.OBJS(INC))THEN
              IRLX(I) = -2
              WRITE(GWMOUT,*)'SUBPROBLEM',I,' IS FATHOMED - ',
...
C-----PRINT STATUS OF SUBPROBLEM
  300 IF(BBITPRT.EQ.1)THEN
        IF(IRLX(J).EQ.1)
     &    WRITE(GWMOUT,*)'SUBPROBLEM',J,' IS UNFATHOMED - IT IS RELAXED'
        IF(IRLX(J).EQ.-1)
     &    WRITE(GWMOUT,*)
     &                  'SUBPROBLEM',J,' IS FATHOMED - IT IS INFEASIBLE'
        IF(IRLX(J).EQ.-2)
     &    WRITE(GWMOUT,*)'SUBPROBLEM',J,' IS FATHOMED - IT IS RELAXED',
     &                ' AND INFERIOR'
        IF(IRLX(J).EQ.-3 .AND. INC.EQ.J)
     &    WRITE(GWMOUT,*)'SUBPROBLEM',J,' IS FATHOMED -', 
     &                'IT IS INTEGER AND THE NEW BEST SOLUTION'
        IF(IRLX(J).EQ.-3 .AND. INC.NE.J)
     &    WRITE(GWMOUT,*)'SUBPROBLEM',J,' IS FATHOMED -',
...
C---------------------------------------------------------------------------
      USE GWM1DCV1, ONLY : NBVAR
      USE GWM1BAS1, ONLY : GWMOUT,ONE
      USE GWM1RMS1, ONLY : BBITMAX
...
C        SO THIS SUBPROBLEM IS ACTUALLY FATHOMED - RESET IRLX AND CONTINUE
        IRLX(NSUB) = -4
        WRITE(GWMOUT,*,ERR=990)'SUBPROBLEM',NSUB,' IS FATHOMED - ',
...
C-----------------------------------------------------------------------
      USE GWM1DCV1, ONLY : NFVAR,NEVAR,NBVAR,FVNAME,EVNAME,BVNAME
...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      ENDIF
C
      CALL GWM1BAS1PF('    Objective Function =',1,OBJ)
      CALL GWM1BAS1PF(' ',0,ZERO)
C
...
C---------------------------------------------------------------------------
      USE GWM1DCC1, ONLY : NLBR
      USE GWM1DCV1, ONLY : NFVAR,NEVAR,NBVAR,BVNLIST,BVLIST
      USE GWM1BAS1, ONLY : ZERO,ONE,BIGINF
      USE GWM1RMS1, ONLY : BBITMAX,NCONF,NVF,
...
C***********************************************************************
C   VERSION: 20FEB2005
C   PURPOSE - FACTOR OR SOLVE THE A SYSTEM OF EQUATIONS DEFINED BY BASIS
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      USE GWM1DCV1, ONLY : FVINI,FVDIR,FVNAME,FVBASE,EVNAME,EVBASE,
     &                     BVNAME,BVBASE
      USE GWM1OBJ1, ONLY : SOLNTYP    
      USE GWM1DCC1, ONLY : NLBR
      USE GWM1RMS1, ONLY : SLPVCRIT,SLPZCRIT,BBITPRT,LPITMAX,NONLIN,
...
C-----LOCAL VARIABLES
      ...
      CHARACTER(LEN=200)::LINE,RMNAME,MPSNAME
      LOGICAL(LGT)::NFOUND
...
C-------PROCESS IRM, THE RESPONSE MATRIX CALCULATION FLAG
        IF(IRM.EQ.0. .OR. IRM.EQ.1)THEN
          READ(LOCAT,'(A)',ERR=991)LINE          ! READ RESPONSE-MATRIX FILENAME  
          LLOC=1
          CALL URWORD(LINE,LLOC,INMS,INMF,0,NDUM,RDUM,IOUT,LOCAT)
          RMNAME=LINE(INMS:INMF)
          RMFILE=IGETUNIT(7,95)
          WRITE(IOUT,3010,ERR=990) RMFILE,RMNAME
          IF(IRM.EQ.0)THEN                       ! READ FROM AN EXISTING FILE
            WRITE(IOUT,3060,ERR=990)
            OPEN(RMFILE,FILE=RMNAME,STATUS='OLD',
     &                  FORM='UNFORMATTED',ERR=997)
          ELSEIF(IRM.EQ.1)THEN                   ! WRITE TO A NEW FILE
            WRITE(IOUT,3070,ERR=990)
            OPEN(RMFILE,FILE=RMNAME,STATUS='NEW',
     &                  FORM='UNFORMATTED',ERR=998)
          ENDIF
        ELSEIF(IRM.EQ.2) THEN
...
C-------PROCESS SLPITPRT
        ...
        IF(SLPITPRT.EQ.1)WRITE(IOUT,3220,ERR=990)
C
C-------PROCESS BBITPRT
        IF(NBVAR.EQ.0)BBITPRT=0                  ! SET TO ZERO IF NO BINARIES
        IF(BBITPRT.EQ.0)THEN
...
C
 ...
 3010 FORMAT(1X,/1X,'OPENING RESPONSE-MATRIX FILE ON UNIT ',I4,':',
     1  /1X,A200)
 ...
 3060 FORMAT(1X,/1X,'IRM EQUALS 0: RESPONSE MATRIX WILL BE READ',
     1  ' FROM FILE',/,' SPECIFIED FOR RMNAME.')
 3070 FORMAT(1X,/1X,'IRM EQUALS 1: RESPONSE MATRIX WILL BE',
     1  ' CALCULATED BY GWM',/,' AND WRITTEN TO FILE SPECIFIED',
     2  ' FOR RMNAME.')
 3080 FORMAT(1X,/1X,'IRM EQUALS 2: RESPONSE MATRIX WILL BE',
     1  ' CALCULATED BY GWM',/,' BUT NOT WRITTEN TO THE RESPONSE',
     2  ' FILE.')
 3090 FORMAT(1X,/1X,'PROGRAM STOPPED. IRM MUST EQUAL 0, 1, OR 2,',
     1  ' BUT WAS SET AT: ',I5)
...
C                                                !   NUMBER OF VARIABLES
        ELSEIF(IRM.EQ.0)THEN                     ! RESPONSE MATRIX READ FROM FILE
          CALL SGWM1RMS1PL
...
C-------THIS IS AN ITERATION OF THE SLP ALGORITHM
        IF(SLPINFCNT.EQ.0)THEN                   ! PRIOR ITERATION SUCCEEDED  
          CALL GWM1RMS1SLP_SETBASE               ! SET NEW BASE SOLUTION AND
          CALL GWM1RMS1SLP_SETDEL                ! SET NEW PERTUBATION VALUE
        ELSEIF(SLPINFCNT.GT.0)THEN               ! PRIOR ITERATION FAILED 
          CALL GWM1RMS1SLP_SETDEL                ! SET ONLY PERTUBATION VALUE
        ENDIF
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE SGWM1RMS1PL
C***********************************************************************
C
C  PURPOSE: READ THE RESPONSE MATRIX FROM FILE 
C-----------------------------------------------------------------------
      USE GWM1BAS1, ONLY : GWM1BAS1PS,GWM1BAS1PF
      USE GWM1HDC1, ONLY : GWM1HDC1FPR 
      USE GWM1STC1, ONLY : GWM1STC1FPR
      INTERFACE 
        SUBROUTINE USTOP(STOPMESS)
        CHARACTER STOPMESS*(*)
        END
...
C
     ...
     &'---------------------------------------------------------------'
     &                    ,0,ZERO)
      CALL GWM1BAS1PF('               Solution Algorithm',0,ZERO)
      CALL GWM1BAS1PF(
     &'---------------------------------------------------------------'
     &                    ,0,ZERO)
...
C-----FILE-READING ERROR
 ...
 9910 FORMAT(/,1X,'*** ERROR READING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &2X,'-- STOP EXECUTION (SGWM1RMS1PL)')
      CALL USTOP(' ')
C
      END SUBROUTINE SGWM1RMS1PL
C
C***********************************************************************
      SUBROUTINE GWM1RMS1SLP_SETDEL
C***********************************************************************
      USE GWM1RMS1, ONLY : DINIT,DMIN,DSC
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
...
C
      RETURN
      END SUBROUTINE GWM1RMS1SLP_SETDEL
C
C***********************************************************************
...
C
      RETURN
      END SUBROUTINE GWM1RMS1SLP_SETBASE
C
      END SUBROUTINE GWM1RMS1PL
C
C
C***********************************************************************
...
C-----------------------------------------------------------------------
     ...
     1                     DELINC,SLPITCNT,VARBASE,PGFACT,IBASE,IREF
      ...
      USE GWM1BAS1, ONLY : ZERO,GWMOUT
      USE GWM1OBJ1, ONLY : SOLNTYP
      USE GWM1BAS1, ONLY : GWM1BAS1PF,GWM1BAS1PS
        ...
        ELSEIF(.NOT.FIRSTSIM .AND. .NOT.LASTSIM)THEN
          ...
          ELSE                                   ! FLOW PROCESS FAILED 
            IF(SLPITCNT.GT.0)THEN                ! MUST BE AN SLP BASE SIMULATION
              CALL SGWM1RMS1PP(3)
              CALL GWM1RMS1PP_RESETBASE
            ELSE                                 ! MUST BE FIRST SIMULATION
              IF(.NOT.MFCNVRG)THEN
                CALL GWM1BAS1PF
...
C
C***********************************************************************
      SUBROUTINE SGWM1RMS1PP(IFLG)
C***********************************************************************
C
C  PURPOSE - WRITE OUTPUT FOR GWM1RMS1PP
      INTEGER(I4B)::IFLG
C-----------------------------------------------------------------------
C
      IF(IFLG.EQ.1)THEN
     ...
     &'---------------------------------------------------------------'
     &                    ,0,ZERO)
        CALL GWM1BAS1PS('  Begin Solution Algorithm',0)
        IF(IPERT.LT.0)THEN                       ! THIS IS A REFERENCE RUN
          CALL GWM1BAS1PS('    Running Reference Simulation',0)
          WRITE(IOUT,1000,ERR=990)
     &      'REFERENCE FLOW PROCESS SIMULATION FOR GWM'
        ELSEIF(IPERT.EQ.0 .AND. IREF.EQ.1)THEN   ! THIS IS A BASE AND REF RUN
          CALL GWM1BAS1PS('    Running Flow Process Simulation',0)
          CALL GWM1BAS1PS('      for both Reference and Base ',0)
          WRITE(IOUT,1000,ERR=990)
     &     'FLOW PROCESS SIMULATION FOR GWM FOR BOTH REFERENCE AND BASE'
        ELSEIF(IPERT.EQ.0 .AND. IREF.EQ.0)THEN   ! THIS IS JUST A BASE RUN
          CALL GWM1BAS1PS('    Running Base Flow Process Simulation',0)
          WRITE(IOUT,1000,ERR=990)'BASE FLOW PROCESS SIMULATION FOR GWM'
        ENDIF
      ELSEIF(IFLG.EQ.2)THEN
        IF(NONLIN)
     &    CALL GWM1BAS1PS('  SLP Algorithm: Begin Iteration ',
     &                  SLPITCNT+1)
        CALL GWM1BAS1PS('    Running Base Flow Process Simulation',0)
        WRITE(IOUT,1000,ERR=990)
     &                            'BASE FLOW PROCESS SIMULATION FOR GWM'
      ELSEIF(IFLG.EQ.3)THEN
        CALL GWM1BAS1PS('      Flow Process Failed: Reset Base',0)
      ELSEIF(IFLG.EQ.4)THEN
        WRITE(IOUT,1000,ERR=990)
     &                      'FINAL FLOW PROCESS SIMULATION FOR GWM'
      ELSEIF(IFLG.EQ.5)THEN
        CALL GWM1BAS1PS('    ',0)
        CALL GWM1BAS1PS('    Calculating Response Matrix',0)
      ELSEIF(IFLG.EQ.6)THEN
        CALL GWM1BAS1PS('      Perturb Flow Variable',IPERT)
        CALL GWM1BAS1PF('       By Perturbation Value:',1,DELINC(IPERT))
        WRITE(IOUT,1010,ERR=990)
     &            'FLOW PROCESS SIMULATION FOR GWM PERTURBATION',IPERT
      ENDIF
C
 1000 FORMAT(/,T20,A,/,T20,38('-'))
...
C-----FILE-WRITING ERROR
 ...
 9900 FORMAT(/,1X,'*** ERROR WRITING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &7X,'SPECIFIED FILE ACCESS: ',A,/
     &7X,'SPECIFIED FILE ACTION: ',A,/
     &2X,'-- STOP EXECUTION (GWM1RMS1PP)')
      CALL USTOP(' ')
C
      END SUBROUTINE SGWM1RMS1PP
C
C***********************************************************************
      SUBROUTINE GWM1RMS1PP_RESETDEL
C***********************************************************************
C  VERSION: 20FEB2005
...
C
      RETURN
      END SUBROUTINE GWM1RMS1PP_RESETDEL
C
C***********************************************************************
      SUBROUTINE GWM1RMS1PP_RESETBASE
C***********************************************************************
C  VERSION: 20FEB2005
...
C
      RETURN
      END SUBROUTINE GWM1RMS1PP_RESETBASE
C
...
C
        ELSEIF(IPGNA.GT.0) THEN            ! PERTURBATION GENERATION FAILED
          ...
          IF(NPGNA.GT.NPGNMX)THEN          ! TOO MANY ATTEMPTS; STOP PROGRAM 
            CALL GWM1BAS1PF('PERTURBATION ATTEMPTS EXCEED MAXIMUM'
     &                       ,0,ZERO)
...
C-----FORMULATE THE HEAD CONSTRAINT COEFFICIENTS FOR THE PERTURBED VARIABLE
      CALL GWM1HDC1FP(RSTRT,IPERT)
C
C-----FORMULATE THE STREAM CONSTRAINT COEFFICIENTS FOR THE PERTURBED VARIABLE
      CALL GWM1STC1FP(RSTRT,IPERT)
C
C-----WRITE OUTPUT FOR BASE SIMULATIONS
      IF(IPERT.EQ.0)THEN                   ! THIS IS A BASE SIMULATION
        IF(SOLNTYP.EQ.'LP' .OR. SLPITCNT.EQ.0 .OR. SLPITPRT.EQ.1)THEN
          CALL GWM1RMS1OT(1)               ! WRITE CONSTRAINTS STATUS
        ENDIF
      ENDIF
C
...
C-----FILE-WRITING ERROR
 ...
 9900 FORMAT(/,1X,'*** ERROR WRITING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &7X,'SPECIFIED FILE ACCESS: ',A,/
     &7X,'SPECIFIED FILE ACTION: ',A,/
     &2X,'-- STOP EXECUTION (GWM1RMS1FP)')
      CALL USTOP(' ')
C
      CONTAINS  
C***********************************************************************
      SUBROUTINE SGWM1RMS1FP
C***********************************************************************
C
...
C-----------------------------------------------------------------------
      ...
      USE GWM1HDC1, ONLY : HDCRHS,NHB,NDD,NDF,NGD,HDCDIR
      USE GWM1BAS1, ONLY : GWM1BAS1PS
      INTEGER IPERT
C-----LOCAL VARIABLES
      REAL(DP)::XNUM
      INTEGER(I4B)::IROW,LNUM,LNUMMAX
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C  
C-----CHECK IF MODFLOW FAILED TO CONVERGE
      IF(.NOT.MFCNVRG)THEN
        CALL GWM1BAS1PS('        Perturbation Failed: ',0)
        CALL GWM1BAS1PS('          Flow Process Did Not Converge',0)
...
C-----CHECK IF RESPONSE COEFFICIENTS HAVE SUFFICIENT PRECISION
  ...
  100   ENDDO
        DO 200 IROW = 1,STCNUM                   ! LOOP OVER STC ROWS
          ...
          ENDIF
  200   ENDDO
        IF(LNUMMAX.LT.NSIGDIG)THEN               ! INSUFFICIENT PRECISION
          CALL GWM1BAS1PS('        Perturbation Failed: ',0)
          CALL GWM1BAS1PS('          Response Precision Inadequate',0)
...
C
      RETURN
      END SUBROUTINE SGWM1RMS1FP
C
      END SUBROUTINE GWM1RMS1FP
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS1FM 
C***********************************************************************
C  VERSION: 20FEB2005
...
C-----------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ZERO
      USE GWM1RMS1, ONLY : IRM,AMAT,DELINC,NRMC,NCON,NDV,HCLOSEG
      USE GWM1DCV1, ONLY : NFVAR,GWM1DCV1FM
      USE GWM1OBJ1, ONLY : GWM1OBJ1FM 
      USE GWM1HDC1, ONLY : GWM1HDC1FM 
      USE GWM1STC1, ONLY : GWM1STC1FM
      USE GWM1SMC1, ONLY : GWM1SMC1FM
      USE GWM1DCC1, ONLY : GWM1DCC1FM
C-----LOCAL VARIABLES
...
C-----FORMULATE THE HEAD CONSTRAINTS
      CALL GWM1HDC1FM(RSTRT,NSLK)
C
C-----FORMULATE THE STREAM CONSTRAINTS
      CALL GWM1STC1FM(RSTRT,NSLK)
C
C-----FORMULATE THE SUMMATION CONSTRAINTS
      CALL GWM1SMC1FM(RSTRT,NSLK)
C
C-----FORMULATE THE DECISION VARIABLE CONSTRAINTS
      CALL GWM1DCC1FM(RSTRT,NSLK)
C
      RETURN
C
      CONTAINS
C***********************************************************************
      SUBROUTINE SGWM1RMS1FM
C***********************************************************************
C
...
C
      RETURN
      END SUBROUTINE SGWM1RMS1FM
C
      END SUBROUTINE GWM1RMS1FM   
C
C
C***********************************************************************
...
C-----------------------------------------------------------------------
      ...
      USE GWM1BAS1, ONLY : GWM1BAS1PS
      USE GWM1RMS1, ONLY : NRMC,NCON, NV, NDV,LPITMAX,AMAT,CST,BNDS,
     &                     RHS,OBJ,SLPITCNT,SLPITMAX,NCONF,NVF
      USE GWM1DCV1, ONLY : NFVAR,NEVAR,NBVAR,FVBASE,FVDIR,FVNAME,
     &                     FVMIN,FVMAX,EVMIN,EVMAX
      USE GWM1OBJ1, ONLY : SOLNTYP,OBJTYP
      USE GWM1RMS1, ONLY : LASTLP
...
C
      IF(SOLNTYP.EQ.'NS')THEN
        CALL GWM1BAS1PS('    ',0)
        CALL GWM1BAS1PS('    Response Matrix File Has Been Written',0)
        CALL USTOP('  ')
      ELSEIF(SOLNTYP.EQ.'MPS')THEN
        CALL GWM1BAS1PS('    ',0)
        CALL GWM1BAS1PS('    Begin Writing MPS File',0)
        CALL GWM1RMS1MPS
        CALL GWM1BAS1PS('    ',0)
        CALL GWM1BAS1PS('    MPS File Has Been Written',0)
        CALL USTOP('  ')
      ELSEIF(SOLNTYP.EQ.'LP')THEN
        LASTLP = .TRUE.
        CALL SGWM1RMS1AP                    ! SOLVE LP
        CALL GWM1RMS1LP_CHKSOL(IFLG,GWMCNVRG)! CHECK STATUS OF SOLUTION
      ...
      ELSEIF(SOLNTYP.EQ.'SLP')THEN
        ...
        CALL SGWM1RMS1AP                    ! SOLVE LP AT THIS ITERATION
        CALL GWM1RMS1SLP(IFLG,OBJ,GWMCNVRG) ! CHECK STATUS OF SOLUTION
      ...
      DO 140 I=1,NCON                            ! LOOP OVER ALL ROWS
        DO 120 II=I+1,NCON                       ! COMPARE WITH ALL LOWER ROWS
          ...
          IF(J.EQ.NDV+1)THEN                     ! ALL ELEMENTS SHOULD BE IDENTICAL
            CALL GWM1BAS1PS('    ',0)
            CALL GWM1BAS1PS('  WARNING:     CONSTRAINT NUMBER',I)
            CALL GWM1BAS1PS('  IDENTICAL TO CONSTRAINT NUMBER',II)
            CALL GWM1BAS1PS(
...
C-----CALL SOLVER
      CALL GWM1BAS1PS('    ',0)
      CALL GWM1BAS1PS('    Solving Linear Program',0)
...
C---------------------------------------------------------------------------
      USE GWM1BAS1, ONLY : MPSFILE,ONE,ZERO
      USE GWM1RMS1, ONLY : NRMC,NCON,NV,NDV,AMAT,CST,BNDS,RHS
      USE GWM1DCV1, ONLY : NFVAR,NEVAR,NBVAR,FVNAME,EVNAME,BVNAME,
...
C-----FILE-WRITING ERROR
 ...
 9900 FORMAT(/,1X,'*** ERROR WRITING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &7X,'SPECIFIED FILE ACCESS: ',A,/
     &7X,'SPECIFIED FILE ACTION: ',A,/
     &2X,'-- STOP EXECUTION (GWM1RMS1MPS)')
      CALL USTOP(' ')
C
      END SUBROUTINE GWM1RMS1MPS
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS1SLP (IFLG,OBJ,GWMCNVRG)
C***********************************************************************
C  VERSION: 20FEB2005
...
C-----------------------------------------------------------------------
      ...
      USE GWM1RMS1, ONLY : CST,SLPVCRIT,SLPZCRIT,SLPINFCNT,SLPITPRT,
...
C-------TEST FOR CONVERGENCE
        ...
        IF(SLPITPRT.EQ.1)THEN                    ! WRITE ITERATION STATUS
          CALL GWM1BAS1PS('    Optimal Solution Found',0)
          CALL GWM1BAS1PF('    Objective Value',1,OBJ)
          IF(NFVAR.GT.0)
     &    CALL GWM1BAS1PF('    Maximum Relative Change in Flow Variable'
     &                    ,1,FVDMAX)
          IF(NEVAR.GT.0)
     &    CALL GWM1BAS1PF('    Max Relative External Variable Change'
     &                    ,1,EVDMAX)
          IF(NBVAR.GT.0)
     &    CALL GWM1BAS1PF('    Maximum Binary Variable Change',1,BVDMAX)
          CALL GWM1BAS1PS('  SLP Algorithm: End Iteration',SLPITCNT)
        ENDIF
        IF(GWMCNVRG)THEN                          ! CONVERGENCE ACHIEVED
          IF(SLPITPRT.EQ.1)
     &    CALL GWM1BAS1PS('  Iterations have converged',0)
...
C---------TERMINATE 
          CALL GWM1BAS1PS('  SLP has failed too many times',0)
          CALL USTOP(' ')
        ENDIF
      ENDIF
C
      RETURN
      END SUBROUTINE GWM1RMS1SLP  
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS1LP_CHKSOL(IFLG,GWMCNVRG)
C***********************************************************************
C  VERSION: 20FEB2005
...
C-------PROBLEM IS INFEASIBLE
        CALL GWM1BAS1PS('  No Feasible Solution Found',0)
        CALL USTOP(' ')
      ELSEIF(IFLG.EQ.2)THEN
C-------SOLUTION HAS BEEN FOUND 
        CALL GWM1BAS1PS('  Problem is Unbounded',0)
        CALL USTOP(' ')
      ENDIF
C
      RETURN
      END SUBROUTINE GWM1RMS1LP_CHKSOL
C
C
C***********************************************************************
...
C-----------------------------------------------------------------------
      ...
      USE GWM1RMS1, ONLY : CST,RHS,BNDS,NRMC,NCON,NV,NDV,NONLIN,RHSIN,
     ...
     4                     CSTROR,CSTRLB,CSTRUB,CONTYP
      ...
      USE GWM1BAS1, ONLY : GWM1BAS1PF
      USE GWM1HDC1, ONLY : GWM1HDC1OT
      USE GWM1STC1, ONLY : GWM1STC1OT
      USE GWM1SMC1, ONLY : GWM1SMC1OT
      USE GWM1DCC1, ONLY : GWM1DCC1OT
      INTEGER(I4B),INTENT(IN)::IFLG
      INTERFACE 
        SUBROUTINE USTOP(STOPMESS)
        CHARACTER STOPMESS*(*)
...
C-----WRITE LINEAR PROGRAM OUTPUT
      ELSEIF(IFLG.EQ.2)THEN    
        ...
        CALL GWM1OBJ1OT                     ! WRITE OBJECTIVE FUNCTION INFO
        RSTRT = 1                           ! SET LOCATION FOR NEXT CONSTRAINT
C
C-------WRITE CONSTRAINTS INFORMATION
        CALL GWM1BAS1PF('       BINDING CONSTRAINTS ',0,ZERO)
        CALL GWM1BAS1PF(
     ...
     &       '---------------        ----     ------      ------------'
     &       ,0,ZERO)
        CALL GWM1HDC1OT(RSTRT,2)            ! WRITE HEAD CONSTRAINT INFO
        CALL GWM1STC1OT(RSTRT,2)            ! WRITE STREAM CONSTRAINT INFO
        CALL GWM1SMC1OT(RSTRT)              ! WRITE SUMMATION CONSTRAINT INFO
        CALL GWM1DCC1OT(RSTRT)              ! WRITE VARIABLE CONSTRAINT INFO
C
C-------WRITE RANGE ANALYSIS INFORMATION
        IF(RANGEFLG.EQ.1)THEN
          WRITE(GWMOUT,2000,ERR=990)
          CALL GWM1BAS1PF(' ',0,ZERO)
          CALL GWM1BAS1PF('       RANGE ANALYSIS ',0,ZERO)
...
C
        ELSE
          WRITE(GWMOUT,6000,ERR=990)
          CALL GWM1BAS1PF(' ',0,ZERO)
          CALL GWM1BAS1PF('       RANGE ANALYSIS NOT REPORTED ',0,ZERO)
        ENDIF
C
...
C
      END SUBROUTINE GWM1RMS1OT
C
C
      END MODULE GWM1RMS1SUBS
C
C
C
C
