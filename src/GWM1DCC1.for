...
C     VERSION: 20FEB2005
      IMPLICIT NONE
      PRIVATE
      PUBLIC::NLBR,GWM1DCC1AR,GWM1DCC1FM,GWM1DCC1OT
C
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE GWM1DCC1AR(FNAME,IOUT,NFVAR,NEVAR,NBVAR,NV,NCON)
C***********************************************************************
C     VERSION: 20FEB2005
C     PURPOSE: READ INPUT FROM THE DECISION-VARIABLES CONSTRAINT FILE
C-----------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ZERO,SMALLEPS,BIGINF
      USE GWM1DCV1, ONLY : FVMIN,FVMAX,FVBASE,FVDIR,EVMIN,EVMAX,FVNAME,
...
C-------PROCESS FVMIN AND FVMAX, THE UPPER AND LOWER BOUNDS ON FLOW
        IF(TPMX.EQ.TPMN)THEN   
          TPMX=TPMX+TPMX*SQRT(SMALLEPS)          ! SEPARATE BOUNDS FROM ZERO
        ELSEIF(TPMX.LE.TPMN)THEN
          WRITE(IOUT,3200,ERR=990)TFVNAME        ! INVALID RELATION
          CALL USTOP(' ')
        ELSEIF(TPMN.LT.ZERO)THEN
          WRITE(IOUT,3300,ERR=990)TFVNAME        ! LOWER BOUND INVALID
          CALL USTOP(' ')
        ELSEIF(TPMX.GT.BIGINF)THEN
          WRITE(IOUT,3400,ERR=990)TFVNAME,BIGINF ! UPPER BOUND INVALID
          CALL USTOP(' ')
        ELSEIF(TPMN.GT.ZERO .AND. NBVAR.EQ.0)THEN
          WRITE(IOUT,3500,ERR=990)TFVNAME
          TPMN = ZERO
        ENDIF
        FVMIN(JJ)=TPMN
        FVMAX(JJ)=TPMX
...
C-----PROCESS EVMIN AND EVMAX, THE UPPER AND LOWER BOUNDS ON EXTERNAL VARIABLES
        IF(TEVMAX.EQ.TEVMIN)THEN
          TEVMAX=TEVMAX+TEVMAX*SQRT(SMALLEPS)    ! SEPARATE BOUNDS FROM ZERO
        ELSEIF(TEVMAX.LT.TEVMIN)THEN
          WRITE(IOUT,4100,ERR=990)TEVNAME        ! INVALID RELATION
          CALL USTOP(' ')
        ELSEIF(TEVMIN.LT.ZERO)THEN
          WRITE(IOUT,4200,ERR=990)TEVNAME        ! LOWER BOUND INVALID
          CALL USTOP(' ')
        ELSEIF(TEVMAX.GT.BIGINF)THEN
          WRITE(IOUT,4300,ERR=990)TEVNAME,BIGINF ! UPPER BOUND INVALID
          CALL USTOP(' ')
        ELSEIF(TEVMIN.GT.ZERO .AND. NBVAR.EQ.0)THEN
          WRITE(IOUT,4400,ERR=990)TEVNAME
          TEVMIN = ZERO
        ENDIF
        EVMIN(JJ)=TEVMIN
        EVMAX(JJ)=TEVMAX
...
C
 ...
 3300 FORMAT(1X,/1X,'PROGRAM STOPPED. WELL ',A10,' HAS A MINIMUM',
 ...
 3500 FORMAT(1X,/1X,' VARIABLE ',A10,' HAS A NON-ZERO LOWER BOUND',
 ...
 4200 FORMAT(1X,/1X,'PROGRAM STOPPED. WELL ',A10,' HAS A MINIMUM',
 ...
 4300 FORMAT(1X,/1X,'PROGRAM STOPPED. WELL ',A10,' HAS A MAXIMUM',
     ...
     2  ' VALUE',/,' ASSIGNED TO MACHINE INFINITY IN GWM: ',D5.2)
 4400 FORMAT(1X,/1X,' VARIABLE ',A10,' HAS A NON-ZERO LOWER BOUND',
     1  ' SPECIFIED',/,' BUT NO BINARY VARIABLES ARE DEFINED',
...
C-----------------------------------------------------------------------
      ...
      USE GWM1DCV1, ONLY : NFVAR,NEVAR,NBVAR,FVMAX,FVMIN,EVMAX,EVMIN,
      ...
      DO 110 I=1,NBVAR
        ...
        DO 100 J=1,BVNLIST(I)
          ...
          IF(VMIN.GT.ZERO)THEN                   ! NEED A CONSTRAINT
            ...
            CONEQU(IROW)=.FALSE.                 ! SET LOGICAL FOR OUTPUT
          ENDIF
  100   ENDDO
  110 ENDDO
C
...
C
      RETURN
      END SUBROUTINE GWM1DCC1FM
C
C
C*************************************************************************
      SUBROUTINE GWM1DCC1OT(RSTRT)
C*************************************************************************
C     VERSION: 20FEB2005
C     PURPOSE: WRITE THE STATUS OF BINDING VARIABLE BOUND CONSTRAINTS 
C---------------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ZERO,ONE
      USE GWM1DCV1, ONLY : NFVAR,NEVAR,NBVAR,FVNAME,FVMAX,FVMIN,
     &                     EVNAME,EVMAX,EVMIN,BVNAME,BVNLIST,BVLIST
      USE GWM1RMS1, ONLY : CST,BNDS,RHS,RANGENAME,NDV
...
C
     ...
     &    (ABS(CST(I)).EQ.FVMIN(I)))THEN  ! VARIABLE AT LOWER BOUND
          ...
          CALL GWM1BAS1CS(CTYPE,FVNAME(I),ZERO,0,0) 
...
C
     ...
     &    (ABS(CST(IEV)).EQ.EVMIN(I)))THEN  ! VARIABLE AT LOWER BOUND
          ...
          CALL GWM1BAS1CS(CTYPE,EVNAME(I),ZERO,0,0) 
