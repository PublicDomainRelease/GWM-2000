...
C     VERSION: 20FEB2005
      ...
      PUBLIC::GWM1STC1AR,GWM1STC1OS,GWM1STC1FP,GWM1STC1FPR,GWM1STC1FM,
     &        GWM1STC1OT
C
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE GWM1STC1AR(FNAME,IOUT,NPER,NV,NCON,NRMC)
C***********************************************************************
C   PURPOSE: READ INPUT FROM THE STREAM CONSTRAINTS FILE
C-----------------------------------------------------------------------
      USE GWM1RMS1, ONLY : IREF
...
C-----------------------------------------------------------------------
      ...
      REAL(SP),INTENT(IN)::STRM(11,MXSTRM)
      INTEGER(I4B),INTENT(IN)::ISTRM(5,MXSTRM)
C-----LOCAL VARIABLES
...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   80     ENDDO
          GOTO 100
   90     CONTINUE
          STCSTATE(I) = REAL(STRM(9,LOC),DP)     ! LOAD STREAMFLOW VALUE
        ENDIF
  100 ENDDO
C
      RETURN
      END SUBROUTINE GWM1STC1OS
C
C
C***********************************************************************
...
C-------THIS IS A REFERENCE SIMULATION
        CALL SGWM1STC1FP                         ! MODIFY CONSTRAINTS
...
C---------IF SAVING RESPONSE MATRIX WRITE THE INITIAL STATE TO FILE
          IF(IRM.EQ.1)WRITE(RMFILE)STCSTATE0(ISTC)
C
...
C---------WRITE RESPONSE MATRIX ELEMENT TO DISK
          IF(IRM.EQ.1) WRITE(RMFILE)RESMAT(ROW,IPERT)
  200   ENDDO
C
      ENDIF
C
...
C-----FOR STREAM CONSTRAINTS, CONVERT STREAM DEPLETION TO STREAM FLOW
      DO 200 I=NSF+1,NSF+NSD
        IF(IRM.EQ.1)WRITE(RMFILE)STCSTATE(I)      ! WRITE REFERENCE STATE
...
C
      RETURN
      END SUBROUTINE SGWM1STC1FP
C
      END SUBROUTINE GWM1STC1FP
C
C
C***********************************************************************
...
C
      RETURN
      END SUBROUTINE SGWM1STC1FPR
C
      END SUBROUTINE GWM1STC1FPR
C
C
C***********************************************************************
...
C
      RETURN
      END SUBROUTINE GWM1STC1FM
C
C
C***********************************************************************
      SUBROUTINE GWM1STC1OT(RSTRT,IFLG)
C*************************************************************************
C  VERSION: 20FEB2005
C  PURPOSE - WRITE STATUS OF CONSTRAINT
C---------------------------------------------------------------------------
      USE GWM1BAS1, ONLY : ZERO,ONE
      USE GWM1RMS1, ONLY : RHS,RANGENAME,NDV
...
C-----WRITE STATUS FOR A FLOW PROCESS SIMULATION
      IF(IFLG.EQ.1)THEN 
        DO 100 ISTC=1,STCNUM
          ...
          ENDIF
          CALL GWM1BAS1CS(CTYPE,STCNAME(ISTC),DIFF,DIRR,1)
100     ENDDO
C
      ...
      ELSEIF(IFLG.EQ.2)THEN
        ...
        ISTC = ISTC + 1
          IF(ABS(RHS(ROW)).GT.ZERO)THEN          ! DUAL VARIABLE IS NON-ZERO 
            ...
            CALL SGWM1STC1OT
            CALL GWM1BAS1CS(CTYPE,STCNAME(ISTC),RHS(ROW),0,0) 
          ENDIF
  200   ENDDO
        RSTRT = RSTRT+STCNUM                     ! SET NEXT STARTING LOCATION
      ENDIF
...
C
      RETURN
      END SUBROUTINE SGWM1STC1OT
C
      END SUBROUTINE GWM1STC1OT
C
C
      END MODULE GWM1STC1