...
C     VERSION: 20FEB2005
     ...
     &        NHB,NDD,NDF,NGD
      PUBLIC::GWM1HDC1AR,GWM1HDC1OS,GWM1HDC1FP,GWM1HDC1FPR,GWM1HDC1FM,
     &        GWM1HDC1OT
C
...
C
      CONTAINS
C*************************************************************************
      SUBROUTINE GWM1HDC1AR(FNAME,IOUT,NPER,NROW,NCOL,NLAY,NV,NCON,NRMC)
C*************************************************************************
C     VERSION: 20FEB2005
C     PURPOSE: READ INPUT FROM THE HEAD CONSTRAINTS FILE 
C---------------------------------------------------------------------------
      USE GWM1BAS1, ONLY : BIGINF
...
C-------THIS IS A REFERENCE SIMULATION
        CALL SGWM1HDC1FP                         ! MODIFY CONSTRAINTS
...
C---------IF SAVING RESPONSE MATRIX WRITE THE INITIAL STATE TO FILE
          IF(IRM.EQ.1)WRITE(RMFILE)HDCSTATE0(IHDC)
C
...
C-----FOR HEAD CONSTRAINTS, CONVERT DRAWDOWN TO HEAD BOUND
      DO 200 I=NHB+1,NHB+NDD
        IF(IRM.EQ.1)WRITE(RMFILE)HDCSTATE(I)      ! WRITE REFERENCE STATE
...
C
      RETURN
      END SUBROUTINE SGWM1HDC1FP
C
      END SUBROUTINE GWM1HDC1FP
C
C
C***********************************************************************
...
C
      RETURN
      END SUBROUTINE SGWM1HDC1FPR
C
      END SUBROUTINE GWM1HDC1FPR
C
C
C***********************************************************************
...
C
      RETURN
      END SUBROUTINE GWM1HDC1FM
C
C
C***********************************************************************
      SUBROUTINE GWM1HDC1OT(RSTRT,IFLG)
C***********************************************************************
C     VERSION: 20FEB2005
...
C-----WRITE STATUS FOR A FLOW PROCESS SIMULATION
      IF(IFLG.EQ.1)THEN            
        DO 100 IHDC=1,HDCNUM
          ...
          ENDIF
          CALL GWM1BAS1CS(CTYPE,HDCNAME(IHDC),DIFF,DIRR,1)
  100   ENDDO
C
      ...
      ELSEIF(IFLG.EQ.2)THEN
        ...
        DO 200 ROW=RSTRT,RSTRT+HDCNUM-1 
          ...
          IF(ABS(RHS(ROW)).GT.ZERO)THEN          ! DUAL VARIABLE IS NON-ZERO 
            ...
            CALL SGWM1HDC1OT
            CALL GWM1BAS1CS(CTYPE,HDCNAME(IHDC),RHS(ROW),0,0) 
          ENDIF
  200   ENDDO
        RSTRT = RSTRT+HDCNUM                     ! SET NEXT STARTING LOCATION
      ENDIF
...
C
      RETURN
      END SUBROUTINE SGWM1HDC1OT
C 
      END SUBROUTINE GWM1HDC1OT
C
C
      END MODULE GWM1HDC1